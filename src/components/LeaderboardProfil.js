...
/**
 * Composant de classement pour le profil utilisateur
 * Affiche TOUS les joueurs et garantit que l'utilisateur connecté apparaît
 */
const LeaderboardProfil = ({ style }) => {
  const { user } = useAuth();
  ...
  const loadLeaderboard = async () => {
    ...
    try {
      ...
      if (activeTab === "global") {
        // Récupérer TOUS les joueurs sans limite
        const rawData = await getGlobalLeaderboard(1000);
        ...
        const userInList = data.find((entry) => entry.userId === user.id);
        if (!userInList) {
          // Si l'utilisateur n'est pas dans la liste, récupérer ses données et les ajouter
          try {
            ...
            const userStats = await getUserAllGameStats(user.id);
            const totalPoints = Object.values(userStats).reduce(
              (sum, gameStats) => sum + (gameStats.totalPoints || 0),
              0
            );

            // Si l'utilisateur a des points, l'ajouter au classement
            if (totalPoints > 0) {
              const userRankData = await getUserGlobalRank(user.id);

              const userEntry = {
                userId: user.id,
                username:
                  userData.username || user.displayName || user.email || "Vous",
                avatar:
                  userData.photoURL ||
                  userData.avatar ||
                  userData.avatarUrl ||
                ...
                rank: userRankData?.rank || data.length + 1,
                isCurrentUser: true,
              };

              // Insérer l'utilisateur à sa position correcte
              const insertIndex = data.findIndex(
                (entry) => entry.rank > userEntry.rank
              );
              if (insertIndex === -1) {
                data.push(userEntry);
              } else {
        ...
        // Afficher TOUS les joueurs sans limite
